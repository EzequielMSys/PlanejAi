...
function validarAtividade(body) {
  ...
  return {
    titulo: titulo.trim(), descricao: body.descricao?.trim() || null, prazo: body.prazo || null, status,
    ...
    questoes: questoes.map((questao, index) => {
      ...
      return { id: questao.id || `q${index + 1}`, enunciado: questao.enunciado.trim(), tipo, opcoes, resposta_correta: questao.resposta_correta ?? null, pontos: Number(questao.pontos) || 1 };
    })
  };
}

function semGabarito(atividade) { return { ...atividade, questoes: atividade.questoes.map(({ resposta_correta, ...questao }) => questao) }; }

function corrigirAutomaticamente(questoes, respostas) {
  let total = 0; let obtidos = 0; let requerManual = false;
  for (const questao of questoes) {
    const pontos = Number(questao.pontos) || 1; total += pontos;
...
async function listar(req, res) { try { return res.json(eGestor(req) ? await atividadeModel.listarGestao() : await atividadeModel.listarPublicadas()); } catch (error) { console.error('[ATIVIDADES]', error); return res.status(500).json({ message: 'Erro ao listar atividades.' }); } }
async function criar(req, res) { try { const dados = validarAtividade(req.body); return res.status(201).json({ atividade: await atividadeModel.criar({ ...dados, criadoPor: usuarioId(req) }) }); } catch (error) { return res.status(400).json({ message: error.message }); } }
...
async function obter(req, res) {
  try { const atividade = await atividadeModel.buscarPorId(req.params.idAtividade); if (!atividade) return res.status(404).json({ message: 'Atividade não encontrada.' }); const resposta = await respostaModel.obterPorAtividadeEUsuario(atividade.id_atividade, usuarioId(req)); return res.json({ atividade: eGestor(req) ? atividade : semGabarito(atividade), minhaResposta: resposta }); } catch (error) { return res.status(500).json({ message: 'Erro ao obter atividade.' }); }
}
module.exports = { listar, criar, atualizar, responderAtividade, entregas, corrigirEntrega, obter };
