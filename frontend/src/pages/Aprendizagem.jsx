...
export default function Aprendizagem() {
  ...
  async function iniciarSimulado(opcoes = { quantidade: 10 }) {
    try {
      const lista = await aprendizagem.simulado(opcoes)
  ...
  async function confirmarResposta() {
    ...
    try {
      const resultado = await aprendizagem.responder(questao.id_questao, resposta, Math.round((Date.now() - inicioQuestao.current) / 1000), questao.embaralhamento)
  ...
  function proximaQuestao() {
    ...
    setIndice((i) => i + 1); setResposta(null); setFeedback(null); inicioQuestao.current = Date.now()
  }
  ...
  return <main className="min-h-screen bg-[#F7F4FA] px-4 py-6 text-[#21162F] dark:bg-[#120E18] dark:text-white sm:px-8">
    <div className="mx-auto max-w-6xl">
      <motion.header initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="workspace-hero relative overflow-hidden rounded-[2.2rem] border border-[#8052D5]/20 bg-white p-7 dark:bg-[#1C1626] sm:p-10">
        ...
        <button onClick={() => iniciarSimulado({ quantidade: 10, minutos: 15 })} className="mt-7 rounded-full bg-[#6D3EC5] px-6 py-3 text-sm font-black text-white shadow-lg shadow-[#6D3EC5]/20 transition hover:-translate-y-0.5">Começar simulado</button>
      </motion.header>
